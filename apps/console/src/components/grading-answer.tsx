 ...
 *
 * Auto-graded answers render read-only beside the ones being marked, so an
 * instructor can see how the rest of the attempt went and can tell at a glance
 * which questions they are and aren't deciding.
 */
    ...
    <li className="border-hairline bg-card rounded-xs border">
        ...
        >
          <div className="flex flex-wrap items-end gap-3">
            <div className="w-28 space-y-1.5">
              ...
              <Input
                ...
                value={points}
                onChange={(e) =>
                  // Clamped here as well as on the server, which refuses
                  // out-of-range with assessment.invalid_points — finding that
                  // out after typing is a worse experience than being unable to.
                  setPoints(
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-muted-foreground text-xs">
              The learner sees this. A bare score explains nothing — it&apos;s the reason marking by
              hand is worth the wait.
            </p>
          </div>
      ...
      {answer.needsManualGrading && answer.graded && answer.feedback && (
        <p className="border-hairline text-muted-foreground border-t px-4 py-2 text-xs">
          Feedback given: {answer.feedback}
        </p>
      )}
    </li>
  );
